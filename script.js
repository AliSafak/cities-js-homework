...
// Açılır/kapanır fonksiyonlar
btnList.addEventListener('click', toggleListCities);
btnFood.addEventListener('click', toggleFamousForFoods);
btnPlace.addEventListener('click', toggleFamousForPlaces);
btnPlates.addEventListener('click', toggleListPlates);

function toggleListCities() {
    if (citiesList.innerHTML === '') {
        for (const city of cities) {
            citiesList.innerHTML += `<li>şehir : ${city.city}, medarıİftiharı : ${city.famousFor}, ünlü olduğu tip : ${city.type}, plaka : ${city.plateNumber}</li>`;
        }
    } else {
        citiesList.innerHTML = ''; // Listeyi kapat
    }
}

function toggleFamousForFoods() {
    if (foodsList.innerHTML === '') {
        for (const city of cities) {
            if (city.type === 'food') {
                foodsList.innerHTML += `<li class='foods-item'>şehir : ${city.city}, ünlü olduğu yemek : ${city.famousFor}</li>`;
            }
        }
    } else {
        foodsList.innerHTML = ''; // Listeyi kapat
    }
}

function toggleFamousForPlaces() {
    if (placesList.innerHTML === '') {
        for (const city of cities) {
            if (city.type === 'place') {
                placesList.innerHTML += `<li class='place-item'>şehir : ${city.city}, ünlü tarihi yeri : ${city.famousFor}</li>`;
            }
        }
    } else {
        placesList.innerHTML = ''; // Listeyi kapat
    }
}

function toggleListPlates() {
    if (oddPlateNumbersList.innerHTML === '' && evenPlateNumbersList.innerHTML === '') {
        for (const city of cities) {
            if (Number(city.plateNumber) % 2 === 0) {
                evenPlateNumbersList.innerHTML += `<li>şehir : ${city.city}, plakası : ${city.plateNumber}</li>`;
            } else {
                oddPlateNumbersList.innerHTML += `<li>şehir : ${city.city}, plakası : ${city.plateNumber}</li>`;
            }
        }
    } else {
        oddPlateNumbersList.innerHTML = ''; // Çift plakalı listeyi kapat
        evenPlateNumbersList.innerHTML = ''; // Tek plakalı listeyi kapat
    }
}
